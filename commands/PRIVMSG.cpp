...
void Privmsg::execute(Client& client, Arguments args)
{
    ...
    for(size_t i = 0;i < targets.size(); ++i)
    {
        if (targets[i][0] == '#' || targets[i][0] == '&')
        {
            ...
            if (!channel)
                ...
                channel->sending(&client, &message, "PRIVMSG", NULL, &targets[i]);
            return;
        }
        ...
        other_client->sending(RPL_MSG(client.get_prefix(), "PRIVMSG", targets[i], message));
    }
}
